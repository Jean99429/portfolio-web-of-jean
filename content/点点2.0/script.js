...
document.querySelectorAll(".other-explorer").forEach((explorer) => {
  ...
  if (!list || !preview || !buttons.length) return;
  ...
  const updatePanel = (key) => {
    ...
    panels.forEach((panel) => panel.classList.toggle("is-active", panel.dataset.otherPanel === key));
  ...
  if (initial) openDrawer(initial, false);
});
